...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    var RestSet = acc
    this.foreach(cur=>if(p(cur)) RestSet = RestSet.incl(cur))
    RestSet
  }

  def union(that: TweetSet): TweetSet = (that.union(this.left)).union(this.right).incl(elem)

  def mostRetweeted: Tweet = {
    filter(cur => cur.retweets > elem.retweets) match {
      case nonempt : NonEmpty => nonempt.mostRetweeted
      case empt : Empty => elem
    }
  }
